...
found="false"
exp_hash=""
found=$(awk -v usr=$usr1 -f check.awk user.tsv)
...
echo
hash1=$(echo "$pswd1" | sha256sum | awk '{print $1}')
exp_hash=$(awk -v usr=$usr1 -f pswd.awk user.tsv)
c=0
...
exists=$(awk -v usr=$usr1 -f check.awk user.tsv)
...
found="false"
exp_hash=""
found=$(awk -v usr=$usr2 -f check.awk user.tsv)
#username exists then password validation
if [[ $found == "true" ]]; then
#reading the password for username and also getting expected hash
...
exp_hash=$(awk -v usr=$usr2 -f pswd.awk user.tsv)
c=0
...
exists=$(awk -v usr=$usr2 -f check.awk user.tsv)
while [[ $exists == "true" ]];do
